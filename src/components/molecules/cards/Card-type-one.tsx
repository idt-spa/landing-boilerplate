import Image from "next/image";
import Link from "next/link";

type CardTypeOneProps = {
  bgColor?: string;
  outlineColor?: string;
  shadowColor?: string;
  textColor?: string;
  buttonTextColor?: string;
  badgeBg?: string;
  title: string[];
  image: string;
  iconBg?: string;
  iconPathFill?: string;
};

export default function CardTypeOne({
  bgColor = "#f3f3f3",
  outlineColor = "#191a23",
  shadowColor = "rgba(25,26,35,1.00)",
  textColor = "black",
  buttonTextColor = "black",
  badgeBg = "#b9ff66",
  title,
  image,
  iconBg = "black",
  iconPathFill = "#b9ff66",
}: CardTypeOneProps) {
  return (
    <div
      className="flex flex-col md:flex-row w-full items-center justify-between overflow-hidden rounded-[45px]"
      style={{
        backgroundColor: bgColor,
        boxShadow: `0px 5px 0px 0px ${shadowColor}`,
        outline: `1px solid ${outlineColor}`,
      }}
    >
      <div className="inline-flex flex-col items-start justify-center gap-8 md:gap-[90px] p-8 sm:p-12 lg:p-8 xl:p-12">
        <h2
          className="flex flex-col items-start justify-start text-3xl font-medium"
          style={{ color: textColor }}
        >
          {title.map((line, i) => (
            <span
              key={i}
              className="rounded-lg px-2"
              style={{ backgroundColor: badgeBg }}
            >
              {line}
            </span>
          ))}
        </h2>
        <Link href={"#"} className="inline-flex items-center justify-start gap-[15px] font-['Space_Grotesk'] cursor-pointer">
          <svg
            width="41"
            height="41"
            viewBox="0 0 41 41"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <circle cx="20.5" cy="20.5" r="20.5" fill={iconBg}></circle>
            <path
              d="M11.25 24.701C10.5326 25.1152 10.2867 26.0326 10.701 26.75C11.1152 27.4674 12.0326 27.7133 12.75 27.299L11.25 24.701ZM30.7694 16.3882C30.9838 15.588 30.5089 14.7655 29.7087 14.5511L16.6687 11.0571C15.8685 10.8426 15.046 11.3175 14.8316 12.1177C14.6172 12.9179 15.0921 13.7404 15.8923 13.9548L27.4834 17.0607L24.3776 28.6518C24.1631 29.452 24.638 30.2745 25.4382 30.4889C26.2384 30.7033 27.0609 30.2284 27.2753 29.4282L30.7694 16.3882ZM12.75 27.299L30.0705 17.299L28.5705 14.701L11.25 24.701L12.75 27.299Z"
              fill={iconPathFill}
            ></path>
          </svg>
          <span
            className="text-xl leading-7 font-normal "
            style={{ color: buttonTextColor }}
          >
            Learn more
          </span>
        </Link>
      </div>

      <div className="relative w-[200px] h-[200px] flex-shrink-0">
        <Image
          src={image}
          alt="Card visual"
          layout="fill"
          objectFit="contain"
          priority
        />
      </div>
    </div>
  );
}
